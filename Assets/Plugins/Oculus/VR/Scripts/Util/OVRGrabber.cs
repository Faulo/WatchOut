...
public class OVRGrabber : MonoBehaviour {
    ...
    protected void CheckForGrabOrRelease(float prevFlex) {
        if ((m_prevFlex >= grabBegin) && (prevFlex < grabBegin) && OVRInput.Get(OVRInput.NearTouch.PrimaryThumbButtons)) {
            GrabBegin();
        } else if ((m_prevFlex <= grabEnd && prevFlex > grabEnd) || !OVRInput.Get(OVRInput.NearTouch.PrimaryThumbButtons)) {
            GrabEnd();
        }
    }
